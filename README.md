...
- **Pure-Go mesh bridge** (`server/service/mesh/`) paired with a bundled [MyOwnMesh](https://myownmesh.net) daemon (Rust, pinned at `v0.3.1` in `.myownmesh-rev`; riscv64-musl build, supervised by the `S94myownmesh` init.d script).
...
- **Zero-login access from anywhere** — the web UI tunnels over the mesh "sites" plane (no port forwarding or VPN), and mesh roster membership *is* the authentication for mesh viewers. The device's **SSH** is advertised as a second site (a raw TCP tunnel — sshd still does its own auth), so the fleet can shell in and re-deploy a device remotely: map the SSH site in AllMyStuff, then `just deploy localhost <mapped-port>` (see `docs/MESH.md`).
