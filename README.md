...
- **CEC branding** — web UI renamed CEC KVM in every locale, the CEC "critical error" mark as the favicon and login logo (`web/public/sipeed.ico` — upstream filename kept, ours bytes), and that mark reduced to a 16×16 glyph on the device OLED (`tools/logo_generator/cec/`). The chrome is still AllMyStuff's design language (deep-violet dark theme, `#f11ea1` magenta accent, Inter font); only the name and the marks are CEC so far.
- **Pure-Go mesh bridge** (`server/service/mesh/`) paired with a bundled [MyOwnMesh](https://myownmesh.net) daemon (Rust, pinned at `v0.3.2` in `.myownmesh-rev`; riscv64-musl build, supervised by the `S94myownmesh` init.d script).
